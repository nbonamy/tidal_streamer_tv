...
class PlaybackFragment : Fragment() {
  ...
  private fun processStatus(status: Status) {
    ...
    // get track
    val track = status.tracks[status.position]
    if (track.item == null) {
      hideSelf()
      return
    }
    ...
    if (currentMediaId == track.mediaId) {
      return
    }

    // all good!
    view?.visibility = View.VISIBLE
    titleView.text = track.item.title
    artistView.text = track.item.mainArtist()?.name ?: ""

    // album art
    Glide.with(this)
      .load(track.item.imageUrl())
      .centerCrop()
